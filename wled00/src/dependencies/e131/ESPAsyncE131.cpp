...
void ESPAsyncE131::parsePacket(AsyncUDPPacket _packet) {
  ...
  if (memcmp(sbuff->acn_id, ESPAsyncE131::ACN_ID, sizeof(sbuff->acn_id)))
	...
	if (protocol == P_ARTNET) {
		...
		if (sbuff->art_opcode != ARTNET_OPCODE_OPDMX
		    && sbuff->art_opcode != ARTNET_OPCODE_OPSYNC
		    && sbuff->art_opcode != ARTNET_OPCODE_OPPOLL)
			error = true; //not a DMX, sync, or poll packet
  ...
  if (!error) {
    _callback(sbuff, _packet.remoteIP(), protocol, _packet.length());
  }
}